...
def snap_to_midpoints(canvas: Canvas, x: float, y: float, tolerance: float = 20.0) -> Optional[Point]:
...
def snap_to_centers(canvas: Canvas, x: float, y: float, tolerance: float = 30.0) -> Optional[Point]:
    """Snap to the center points of circles, rectangles, arcs, and polygons on the canvas."""
    closest_dist = tolerance
    snap_point = None

    # Check all items that could be shapes
    for item in canvas.find_all():
        # Get the tags for this item
        tags = canvas.gettags(item)
        
        # Skip items that are utility elements
        if 'grid_lines' in tags or 'snap_indicator' in tags or 'snap_label' in tags or \
           'preview_line' in tags or 'start_marker' in tags or 'above_all' in tags or \
           'temp_freehand' in tags or 'preview_polygon' in tags:
            continue
        ...
        if shape_type == 'polygon':
            coords = canvas.coords(item)
            if not coords or len(coords) < 4:  # Need at least 2 points for a valid polygon
                continue
            ...
            if dist < closest_dist:
                closest_dist = dist
                snap_point = (center_x, center_y)
                
        # Process circles, rectangles, and arcs
        elif shape_type in ['oval', 'rectangle', 'arc']:
            coords = canvas.coords(item)
            if not coords or len(coords) < 4:  # Make sure we have valid coordinates
                continue
