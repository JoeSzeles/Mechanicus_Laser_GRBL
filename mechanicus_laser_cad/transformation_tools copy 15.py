...
class TransformationTools:
    ...
    def mirror_along_line(self):
        ...
        # Get original colors before they might be changed by selection
        original_fill = self.canvas.itemcget(self.selected_shape, 'fill')
        original_outline = self.canvas.itemcget(self.selected_shape, 'outline')
        original_width = self.canvas.itemcget(self.selected_shape, 'width')
        
        # Shape properties
        kwargs = {}
        if shape_type == 'line':
            kwargs['fill'] = original_fill
            kwargs['width'] = original_width
        else:
            kwargs['fill'] = original_fill
            kwargs['outline'] = original_outline
            kwargs['width'] = original_width
        ...
        if self.create_copy.get():
            ...
            if shape_type == 'line':
                self.original_colors[new_shape] = original_fill
                self.original_widths[new_shape] = original_width
            else:
                self.original_colors[new_shape] = original_outline
                self.original_fills[new_shape] = original_fill
                self.original_widths[new_shape] = original_width
            ...
            if shape_type in ('line', 'polygon'):
                ...
                self.original_colors[self.selected_shape] = original_outline
                self.original_fills[self.selected_shape] = original_fill
                self.original_widths[self.selected_shape] = original_width
