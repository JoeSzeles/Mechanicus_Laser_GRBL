...
class TransformationTools:
    ...
    def mirror_along_line(self):
        ...
        shape_coords = list(self.canvas.coords(self.selected_shape))
        
        # Calculate mirrored coordinates
        mirrored_coords = self.calculate_mirrored_coords(shape_coords, x1, y1, x2, y2)
        ...
        if self.create_copy.get():
            ...
            # Copy all relevant properties from original shape
            if shape_type == 'line':
            ...
            # Copy any custom tags from the original shape (excluding utility tags)
            original_tags = [tag for tag in self.canvas.gettags(self.selected_shape) 
                           if tag not in ['current', 'selected', 'bounding_box', 'scale_handle']]
            ...
            # Apply the original tags to the new shape
            for tag in original_tags:
                self.canvas.addtag_withtag(tag, new_shape)
    ...
    def calculate_mirrored_coords(self, coords, x1, y1, x2, y2):
        ...
        for i in range(0, len(coords), 2):
            ...
            # Project onto normal vector to get distance from line
            dist = vx*nx + vy*ny
            
            # Mirror point by moving twice the distance in the opposite direction of the normal
            mx = px - 2*dist*nx
            my = py - 2*dist*ny
    ...
    def preview_mirror_along_line(self, line):
        ...
        if self.mirror_preview:
            ...
            shape_type = self.canvas.type(self.selected_shape)
            # Handle different shape types for preview
            if shape_type == 'line':
                ...
                self.mirror_preview = getattr(self.canvas, f'create_{shape_type}')(
                    *mirrored_coords,
                    fill='',
                    outline='green',
                    width=2,
