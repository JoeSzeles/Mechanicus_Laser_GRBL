...
class TransformationTools:
    ...
    def rotate_shape_around_point(self, shape_id, cx, cy, angle):
        ...
        if shape_type == 'arc':
            # For arcs, just update the start angle and preserve the extent
            start = float(self.canvas.itemcget(shape_id, 'start'))
            extent = float(self.canvas.itemcget(shape_id, 'extent'))
            self.canvas.itemconfig(shape_id, start=start + angle)
            ...
            # Rotate bounding box corners around the rotation center
            points = []
            ...
            # Update arc bounding box
            self.canvas.coords(shape_id, points[0], points[1], points[4], points[5])
