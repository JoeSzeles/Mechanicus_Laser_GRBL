...
class TransformationTools:
    ...
    def show_tools_window(self):
        ...
        if self.tools_window is None or not tk.Toplevel.winfo_exists(self.tools_window):
            ...
            self.tools_window.geometry("200x350")  # Increased height for new button
            ...
            # Main label
            label = tk.Label(self.tools_window, text="Transform Tools", 
                           fg="white", bg="#263d42")
            label.pack(pady=10)
            
            # Select button
            self.select_btn = tk.Button(self.tools_window, text="Select",
            ...
            self.scale_entry_x.insert(0, "10.0")
            ...
            # Deselect button
            deselect_btn = tk.Button(self.tools_window, text="Deselect",
                                   command=self.clear_selection,
                                   fg='white', bg='#263d42',
                                   activebackground='#2ecc71',
                                   activeforeground='white')
            deselect_btn.pack(pady=10)
    ...
    def apply_manual_scale(self):
        ...
        try:
            ...
            size_x_mm = float(self.scale_entry_x.get())
            size_y_mm = float(self.scale_entry_y.get())
            ...
            # Get current shape size in pixels (scaled)
            current_width = self.original_bbox[2] - self.original_bbox[0]
            current_height = self.original_bbox[3] - self.original_bbox[1]
            
            # Convert desired mm sizes to pixels using scale factor
            scale_factor = self.get_scale_factor()
            size_x_pixels = size_x_mm * scale_factor
            size_y_pixels = size_y_mm * scale_factor
            
            # Calculate scale factors
            scale_x = size_x_pixels / current_width
            scale_y = size_y_pixels / current_height
            
            if self.keep_aspect_ratio.get():
                # Use larger scale factor for both dimensions
    ...
    def on_canvas_click(self, event):
        """Modified click handler for group-aware selection"""
    ...
    def apply_scale(self, scale_x, scale_y):
        ...
        if self.scale_entry_x and self.scale_entry_y:
            self.scale_entry_x.delete(0, tk.END)
            self.scale_entry_x.insert(0, f"{scale_x:.2f}")
            self.scale_entry_y.delete(0, tk.END)
            self.scale_entry_y.insert(0, f"{scale_y:.2f}")
    ...
    def get_group_members(self, shape_id):
        """Get all members of a group that contains the given shape"""
        group_tags = [tag for tag in self.canvas.gettags(shape_id) if tag.startswith('group_')]
        if not group_tags:
            return [shape_id]
        
        group_members = []
        for tag in group_tags:
            members = self.canvas.find_withtag(tag)
            group_members.extend(members)
        return list(set(group_members))

    def get_group_bounds(self, group_members):
        """Calculate the bounding box for a group of shapes"""
        if not group_members:
            return None
        
        bounds = self.canvas.bbox(group_members[0])
        x1, y1, x2, y2 = bounds
        
        for shape_id in group_members[1:]:
            shape_bounds = self.canvas.bbox(shape_id)
            if shape_bounds:
                x1 = min(x1, shape_bounds[0])
                y1 = min(y1, shape_bounds[1])
                x2 = max(x2, shape_bounds[2])
                y2 = max(y2, shape_bounds[3])
        
        return (x1, y1, x2, y2)

    def select_group(self, shape_id):
        """Select all shapes in a group"""
        members = self.get_group_members(shape_id)
        self.clear_selection()
        
        for member_id in members:
            shape_type = self.canvas.type(member_id)
            if shape_type == 'line':
                self.original_colors[member_id] = self.canvas.itemcget(member_id, 'fill')
                self.original_widths[member_id] = self.canvas.itemcget(member_id, 'width')
                self.canvas.itemconfig(member_id, fill='green', width=2)
            else:
                self.original_colors[member_id] = self.canvas.itemcget(member_id, 'outline')
                self.original_fills[member_id] = self.canvas.itemcget(member_id, 'fill')
                self.original_widths[member_id] = self.canvas.itemcget(member_id, 'width')
                self.canvas.itemconfig(member_id, outline='green', width=2)
            
            self.selected_shape = member_id
            self.original_coords = list(self.canvas.coords(member_id))
            self.create_bounding_box()
