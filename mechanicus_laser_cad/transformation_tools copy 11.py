...
class TransformationTools:
    ...
    def mirror_along_line(self):
        ...
        # Determine creation method and properties
        if shape_type in ('line', 'polygon'):
            create_method = f'create_{shape_type}'
        else:  # oval, rectangle
            create_method = 'create_polygon'  # Convert to polygon for general mirroring
        ...
        if self.create_copy.get():
            ...
            new_shape = getattr(self.canvas, create_method)(
                *mirrored_coords,
                tags=original_tags,
                **kwargs
            )
