...
class LineEditor:
    ...
    def deactivate_selection(self):
        ...
        self.current_tool = None
        # Reset all button states if tools window exists
        if self.tools_window and tk.Toplevel.winfo_exists(self.tools_window):
            for button in self.tool_buttons.values():
                button.configure(relief='raised', bg='#263d42')
    ...
    def on_window_close(self):
        ...
        self.clear_selection()  # Clear any active selection
    ...
    def update_angle_label(self, x, y, angle):
        ...
        # Delete ALL existing angle labels and their backgrounds
        self.canvas.delete('angle_label')
        
        # Format angle to 1 decimal place
        angle_text = f"{angle:.1f}°"
        
        # Create white background rectangle first
        bbox_padding = 4
        text_width = len(angle_text) * 8  # Approximate width based on text length
        text_height = 16  # Approximate height for font size 12
        
        bg = self.canvas.create_rectangle(
            x + 20 - bbox_padding,
            y - 20 - bbox_padding,
            x + 20 + text_width + bbox_padding,
            y - 20 + text_height + bbox_padding,
            fill='white',
            outline='#666666',
            tags='angle_label'
        )
        
        # Create text label on top of background
        text = self.canvas.create_text(
            x + 20, y - 20,  # Offset from cursor
            text=angle_text,
            fill='black',
            font=('Arial', 12),
            anchor='nw',  # Align to top-left corner
            tags='angle_label'
        )
        
        # Store both the background and text objects
        self.angle_label = [bg, text]
