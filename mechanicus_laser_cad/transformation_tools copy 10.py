...
class TransformationTools:
    ...
    def mirror_along_line(self):
        ...
        if self.create_copy.get():
            ...
            # Copy any custom tags from the original shape (excluding utility and shape ID tags)
            original_tags = [tag for tag in self.canvas.gettags(self.selected_shape) 
                           if not tag.startswith('shape_') and  # Exclude shape IDs
                           tag not in ['current', 'selected', 'bounding_box', 'scale_handle']]
            
            # Add the new shape ID to tags
            original_tags.append(new_shape_id)
            
            # Create the new shape with the new ID and original tags
    ...
    def calculate_mirrored_coords(self, coords, x1, y1, x2, y2):
        ...
        for i in range(0, len(coords), 2):
            ...
            # Calculate mirror point using line direction and normal components
            mx = x1 + along*dx - dist*nx
            my = y1 + along*dy - dist*ny
