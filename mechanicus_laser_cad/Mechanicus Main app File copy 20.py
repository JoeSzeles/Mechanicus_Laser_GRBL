...
import engrave
from snaptools import snap_to_grid, snap_to_endpoints, snap_to_midpoints
import snaptools
...
snap_to_midpoints_var = tk.BooleanVar(value=False)  # Default to False for midpoint snapping
snap_to_centers_var = tk.BooleanVar(value=False)  # Default to False for center snapping
...
snap_tool_window = None  # Store reference to snap tools window
...
def get_snap_point(x, y, is_final=False):
    """Get the nearest snap point based on enabled snap options"""
    global snap_var, snap_to_endpoints_var, snap_to_midpoints_var, snap_to_centers_var
    
    # Use consistent tolerance of 30 pixels for all snap types
    tolerance = 30.0
    
    snap_point = None
    
    # Check center point snap first
    if snap_to_centers_var.get():
        snap_point = snaptools.snap_to_centers(cv, x, y, tolerance)
        if snap_point:
            # Create center snap indicator (blue diamond)
            marker_size = 6
            cv.create_polygon(
                snap_point[0], snap_point[1] - marker_size,  # Top
                snap_point[0] + marker_size, snap_point[1],  # Right
                snap_point[0], snap_point[1] + marker_size,  # Bottom
                snap_point[0] - marker_size, snap_point[1],  # Left
                fill='#0088FF', outline='white',
                tags=('snap_indicator', 'above_all')
            )
            # Create center snap label
            cv.create_text(
                snap_point[0], snap_point[1] - 15,
                text="Center",
                fill='#0088FF',
                font=('Arial', 8, 'bold'),
    ...
    # Then check endpoint snap
    if snap_to_endpoints_var.get():
        snap_point = snaptools.snap_to_endpoints(cv, x, y, tolerance)
        if snap_point:
            # Create endpoint snap indicator (red triangle)
            marker_size = 6
            cv.create_polygon(
                snap_point[0] - marker_size, snap_point[1] + marker_size,
                snap_point[0] + marker_size, snap_point[1] + marker_size,
                snap_point[0], snap_point[1] - marker_size,
                fill='red', outline='white',
                tags=('snap_indicator', 'above_all')
            )
            # Create endpoint snap label
            cv.create_text(
                snap_point[0], snap_point[1] - 15,
                text="Endpoint",
                fill='red',
    ...
    # Then check midpoint snap
    if snap_to_midpoints_var.get():
        snap_point = snaptools.snap_to_midpoints(cv, x, y, tolerance)
        if snap_point:
            # Create midpoint snap indicator (blue triangle)
            marker_size = 6
            cv.create_polygon(
                snap_point[0] - marker_size, snap_point[1] + marker_size,
                snap_point[0] + marker_size, snap_point[1] + marker_size,
    ...
    # Finally check grid snap
    if snap_var.get():
        grid_size = float(grid_size_var.get())
        snap_point = snaptools.snap_to_grid(x, y, grid_size)
        if snap_point:
            # Create grid snap indicator (green triangle)
            marker_size = 6
            cv.create_polygon(
                snap_point[0] - marker_size, snap_point[1] + marker_size,
                snap_point[0] + marker_size, snap_point[1] + marker_size,
...
def activate_Paint_Gcode(event):
    ...
    try:
        if laser_active_var.get():
            # Turn on laser
            power = laser_power_input.get('1.0', 'end-1c')
            print(f"Activating laser with power: {power}")  # Debugging statement
            ser.write(f"M3 S{power}\n".encode())
            ser.flush()
        # Start drawing line
        Paint_Gcode(event, gcode)
    except Exception as e:
        print(f"Error in activate_Paint_Gcode: {e}")  # Error handling
...
def select_line(event):
    ...
    print(f"Items found at position ({x}, {y}): {items}")  # Debugging statement
    
    # Filter for selectable items and get the topmost one
    selected_items = []
    for item in items:
        tags = cv.gettags(item)
        print(f"Item {item} tags: {tags}")  # Debugging statement
...
def start_polygon(event):
    global lastx, lasty, current_polygon, current_shape_id, linecount, hexstr
    x = cv.canvasx(event.x)
    y = cv.canvasy(event.y)
    ...
    # Check for snap points in priority order: centers, endpoints, grid
    if snap_to_centers_var.get():
        center_point = snaptools.snap_to_centers(cv, x, y, tolerance=20.0)
        if center_point:
            x, y = center_point
    elif snap_to_endpoints_var.get():
        endpoint = snaptools.snap_to_endpoints(cv, x, y, tolerance=20.0)
        if endpoint:
            x, y = endpoint
    elif snap_var.get() and grid_var.get():
        x, y = get_snap_point(x, y, is_final=True)
    ...
    cv.create_oval(
        lastx-4, lasty-4, lastx+4, lasty+4,
        fill='#00FF00', outline='white',
        tags=('center_marker', 'above_all')
    )
    
    # Initialize polygon with just the center point
    current_polygon = cv.create_polygon(
        [x, y],  # Just the center point
        outline=hexstr,
        fill='',
        width=int(brush.get("1.0", "end-1c")),
        tags=('preview_polygon', current_shape_id)
    )
    ...
    cv.create_line(
        ...
        fill=hexstr,
        width=int(brush.get("1.0", "end-1c")),
        tags=('preview_line', current_shape_id)
    )
    
    cv.tag_raise('above_all')
    
    # Bind mouse motion for snap indicators
...
def update_polygon(event):
    ...
    if current_polygon:
        ...
        if snap_to_centers_var.get():
            ...
            if center_point:
                x, y = center_point
                # Show blue diamond indicator for center snap
                marker_size = 6
                cv.delete('snap_indicator')
                cv.create_polygon(
                    x, y - marker_size,  # Top
            ...
            if endpoint:
                x, y = endpoint
                # Show red triangle indicator for endpoint snap
                marker_size = 6
                cv.delete('snap_indicator')
                cv.create_polygon(
            ...
            x, y = get_snap_point(x, y, is_final=False)
        
        # Calculate radius and number of sides
        radius = math.sqrt((x - lastx) ** 2 + (y - lasty) ** 2)
        ...
        # Update preview line to show radius
        preview_line = cv.find_withtag('preview_line')
        if preview_line:
            cv.coords(preview_line[0], lastx, lasty, x, y)
...
def finish_polygon(event):
    ...
    if current_polygon:
        ...
        # Get the final radius and calculate points
        radius = math.sqrt((x - lastx) ** 2 + (y - lasty) ** 2)
        sides = 6  # Default to hexagon
        points = []
        for i in range(sides):
        ...
        final_polygon = cv.create_polygon(
            points,
            fill='',
            outline=hexstr,
            width=int(brush.get("1.0", "end-1c")),
            tags=('shape', 'all_lines', current_shape_id)
        )
        
        # Add to active layer if it exists
        if hasattr(cv, 'active_layer'):
            cv.addtag_withtag(f'layer{cv.active_layer}', current_shape_id)
        
        # Clean up
        cv.delete('preview_polygon')
        ...
        if 'layers_window' in globals() and layers_window:
            layers_window.update_layer_items()

def start_line(event):
    global current_shape, current_shape_id, linecount, hexstr
...
def update_line(event):
    ...
    start_x, start_y = cv.coords(current_shape)[:2]
    ...
    # Update preview line
    cv.coords(current_shape, start_x, start_y, x, y)
    
    # Ensure indicators are above other elements
    cv.tag_raise('above_all')

def end_line(event):
    global current_shape, current_shape_id, hexstr
    ...
    cv.create_line(
        ...
        fill=hexstr,
        width=int(brush.get("1.0", "end-1c")),
        tags=('line', 'shape', 'all_lines', current_shape_id)
    )
...
def update_snap_indicator(event):
    """Update snap indicators based on mouse position"""
    ...
    # Get snap point using the same logic as in get_snap_point
    tolerance = 30.0
...
def open_snap_tool_window():
    ...
    if snap_tool_window is not None:
        snap_tool_window.destroy()
    
    snap_tool_window = tk.Toplevel()
    snap_tool_window.title("Snap Tools")
    snap_tool_window.geometry("200x200+1700+100")
...
def select_tool(tool_name):
    ...
    print(f"Selecting tool: {tool_name}")  # Debugging statement
    ...
    try:
        # Convert tool_name to lowercase for case-insensitive comparison
        tool_lower = tool_name.lower()
        
        # Bind events for the selection tool
        if tool_lower == 'select':
            cv.bind('<Button-1>', select_line)  # Start selection
            cv.bind('<ButtonRelease-1>', delete_text)  # Complete selection and allow deletion
        elif tool_lower == 'line':
            cv.bind('<Button-1>', start_line)
            cv.bind('<B1-Motion>', update_line)
            cv.bind('<ButtonRelease-1>', end_line)
            ...
            cv.bind('<B1-Motion>', update_rectangle)
            cv.bind('<ButtonRelease-1>', finish_rectangle)
        elif tool_lower == 'polygon':
            cv.bind('<Button-1>', start_polygon)
            cv.bind('<B1-Motion>', update_polygon)
            cv.bind('<ButtonRelease-1>', finish_polygon)
        elif tool_lower == 'freehand':
            cv.bind('<Button-1>', start_freehand)
            cv.bind('<B1-Motion>', update_freehand)
            cv.bind('<ButtonRelease-1>', finish_freehand)
        elif tool_lower == 'arc':
            ...
            cv.bind('<B1-Motion>', update_arc)
            cv.bind('<ButtonRelease-1>', finish_arc)
        elif tool_lower == 'text':
            cv.bind('<Button-1>', activate_text)
        elif tool_lower == 'paint':
            cv.bind('<Button-1>', activate_Paint_Gcode)
            cv.bind('<B1-Motion>', lambda event: Paint_Gcode(event, gcode))
            cv.bind('<ButtonRelease-1>', key_released)
        elif tool_lower == 'live carving':
            print("Binding Live Carving tool events")  # Debugging statement
            cv.bind('<Button-1>', activate_Paint_Gcode)
            cv.bind('<B1-Motion>', lambda event: Paint_Gcode(event, gcode))
            cv.bind('<ButtonRelease-1>', key_released)
            print("Live Carving tool events bound successfully")  # Debugging statement
    except Exception as e:
        print(f"Error in select_tool: {e}")  # Error handling
