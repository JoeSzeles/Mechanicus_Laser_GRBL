...
snap_var = tk.BooleanVar(value=True)  # Snapping enabled by default
grid_size_var = tk.StringVar(value="5.0")  # Default grid size 5mm
scale_var = tk.StringVar(value="100%")  # Add scale variable if not already defined

# Snap tools variables
active_snap_tools = {
    'Grid Point': tk.BooleanVar(value=False),  # Initially disabled
    'Endpoint': tk.BooleanVar(value=True),     # Active by default
    'Center': tk.BooleanVar(value=True),       # Active by default
    'Middle': tk.BooleanVar(value=True),       # Active by default
    'Intersection': tk.BooleanVar(value=True), # Active by default
    'Tangent': tk.BooleanVar(value=True),      # Active by default
    'Quadrant': tk.BooleanVar(value=True),     # Active by default
    'Parallel': tk.BooleanVar(value=True)      # Active by default
}
snap_tools_window = None
snap_point_type = None
...
def show_snap_indicator(x, y, snap_type):
    """Show a visual indicator for the snap point."""
    cv.delete('snap_indicator')
    
    # Create a more visible dot with highlight
    ...
    # Add larger text label
    cv.create_text(x, y-20, text=snap_type,
                  fill='#000080', font=('Arial', 12, 'bold'),
                  tags=('snap_indicator', 'above_all'))

def get_snap_point(x, y, is_final=False):
    """Get the nearest snap point based on enabled snap tools."""
    global snap_point_type
    snap_point_type = None
    
    # Only proceed if snap is enabled
    if not snap_var.get():
        return x, y

    # Convert screen coordinates to canvas coordinates
    canvas_x = cv.canvasx(x)
    canvas_y = cv.canvasy(y)
    ...
    # Base snap distance (10 pixels at 100% zoom)
    snap_distance = 10.0 / scale_factor

    # Get all items near the cursor for snap types
    nearby_items = cv.find_overlapping(canvas_x-snap_distance, canvas_y-snap_distance, 
                                     canvas_x+snap_distance, canvas_y+snap_distance)
    
    # Filter out temporary items, grid lines, and snap indicators
    valid_items = [item for item in nearby_items 
                  if 'temp' not in cv.gettags(item) 
                  and 'grid_lines' not in cv.gettags(item)
                  and 'potential_snap' not in cv.gettags(item)
                  and 'snap_indicator' not in cv.gettags(item)]

    # Check endpoints first
    if active_snap_tools['Endpoint'].get():
        for item in valid_items:
            coords = cv.coords(item)
            if len(coords) >= 4:
                # Check start point
                d = distance((canvas_x, canvas_y), (coords[0], coords[1]))
                if d < snap_distance:
                    closest_point = (coords[0], coords[1])
                    min_distance = d
                    snap_point_type = 'Endpoint'
                    break  # Exit after finding first valid snap point
                # Check end point
                d = distance((canvas_x, canvas_y), (coords[-2], coords[-1]))
                if d < snap_distance:
                    closest_point = (coords[-2], coords[-1])
                    min_distance = d
                    snap_point_type = 'Endpoint'
                    break  # Exit after finding first valid snap point
    
    # Only continue checking if no endpoint was found
    if not snap_point_type:
        # Check intersections
        if active_snap_tools['Intersection'].get():
            for i, item1 in enumerate(valid_items):
                coords1 = cv.coords(item1)
                if len(coords1) >= 4:
                    for item2 in valid_items[i+1:]:
                        coords2 = cv.coords(item2)
                        if len(coords2) >= 4:
                            try:
                                x1, y1, x2, y2 = coords1[0], coords1[1], coords1[-2], coords1[-1]
                                x3, y3, x4, y4 = coords2[0], coords2[1], coords2[-2], coords2[-1]
                                ...
                                if abs(denominator) > 1e-10:
                                    ...
                                    if 0 <= t <= 1 and 0 <= u <= 1:
                                        ix = x1 + t * (x2 - x1)
                                        iy = y1 + t * (y2 - y1)
                                        d = distance((canvas_x, canvas_y), (ix, iy))
                                        if d < snap_distance:
                                            closest_point = (ix, iy)
                                            min_distance = d
                                            snap_point_type = 'Intersection'
                                            break  # Exit after finding first valid snap point
                            except:
                                continue
                if snap_point_type:
    ...
    # Only continue checking if no intersection was found
    if not snap_point_type:
        # Check center points
        if active_snap_tools['Center'].get():
            for item in valid_items:
                coords = cv.coords(item)
                if len(coords) >= 4:
                    center = ((coords[0] + coords[-2])/2, (coords[1] + coords[-1])/2)
                    d = distance((canvas_x, canvas_y), center)
                    if d < snap_distance:
                        closest_point = center
                        min_distance = d
                        snap_point_type = 'Center'
                        break  # Exit after finding first valid snap point
    
    # Only continue checking if no center was found
    if not snap_point_type:
        # Check middle points
        if active_snap_tools['Middle'].get():
            for item in valid_items:
                coords = cv.coords(item)
                if len(coords) >= 4:
                    middle = ((coords[0] + coords[-2])/2, (coords[1] + coords[-1])/2)
                    d = distance((canvas_x, canvas_y), middle)
                    if d < snap_distance:
                        closest_point = middle
                        min_distance = d
                        snap_point_type = 'Middle'
                        break  # Exit after finding first valid snap point
    
    # Only check grid points if no other snap points were found AND grid is visible AND grid snap is enabled
    if not snap_point_type and grid_var.get() and active_snap_tools['Grid Point'].get():
        try:
            grid_size = float(grid_size_var.get())
            grid_x = round(canvas_x / grid_size) * grid_size
            grid_y = round(canvas_y / grid_size) * grid_size
            d = distance((canvas_x, canvas_y), (grid_x, grid_y))
            
            if d < snap_distance:
                closest_point = (grid_x, grid_y)
                min_distance = d
                snap_point_type = 'Grid Point'
        except ValueError:
            pass

    # Show snap indicator unless this is the final point
    if not is_final and snap_point_type:
        show_snap_indicator(closest_point[0], closest_point[1], snap_point_type)
    else:
        cv.delete('snap_indicator')
...
def on_grid_toggle():
    ...
    if grid_var.get():
        create_grid()
        active_snap_tools['Grid Point'].set(True)  # Enable grid snapping when grid is shown
        cv.delete('potential_snap')  # Clear any existing snap indicators
    else:
        cv.delete('grid_lines')  # Delete grid lines
        active_snap_tools['Grid Point'].set(False)  # Disable grid snapping when grid is hidden
        cv.delete('potential_snap')  # Clear any existing snap indicators
    cv.update()  # Force canvas update
...
def start_circle(event):
    global lastx, lasty, current_circle, circle_center_marker
    ...
    circle_center_marker = cv.create_oval(
        x-4, y-4, x+4, y+4,
    ...
    current_circle = cv.create_oval(
        x, y, x, y,
        outline=hexstr, width=2,
        tags=('preview_circle', 'above_all')
    )
...
def finish_circle(event):
    global current_circle, circle_center_marker, linecount
    ...
    if current_circle:
        ...
        cv.create_oval(
            ...
            lastx + radius, lasty + radius,
            outline=hexstr, width=2,
            tags=(f"shape_{linecount}", "circle")
        )
        
        # Clean up temporary elements
        cv.delete(current_circle)
        cv.delete(circle_center_marker)
        cv.delete('snap_indicator')
        
        # Reset variables
        current_circle = None
        circle_center_marker = None
        linecount += 1
...
def start_rectangle(event):
    global lastx, lasty, current_rectangle, rectangle_start_marker
    
    # Get canvas coordinates
    x = cv.canvasx(event.x)
    y = cv.canvasy(event.y)
    
    # Get snap point for start corner
    if snap_var.get():
        x, y = get_snap_point(x, y, is_final=True)
    ...
    # Create start corner marker
    rectangle_start_marker = cv.create_oval(
        x-4, y-4, x+4, y+4,
        fill='blue', outline='white',
    ...
    current_rectangle = cv.create_rectangle(
        x, y, x, y,
        outline=hexstr, width=2,
        tags=('preview_rectangle', 'above_all')
    )
...
def update_rectangle(event):
    ...
    if current_rectangle:
        ...
        # Get snap point for opposite corner
        if snap_var.get():
            x, y = get_snap_point(x, y)
...
def finish_rectangle(event):
    global current_rectangle, rectangle_start_marker, linecount
    ...
    if current_rectangle:
        ...
        # Get snap point for final corner
        if snap_var.get():
            x, y = get_snap_point(x, y, is_final=True)
        ...
        cv.create_rectangle(
            lastx, lasty, x, y,
            outline=hexstr, width=2,
            tags=(f"shape_{linecount}", "rectangle")
        )
        
        # Clean up temporary elements
        cv.delete(current_rectangle)
        cv.delete(rectangle_start_marker)
        cv.delete('snap_indicator')
        
        # Reset variables
        current_rectangle = None
        rectangle_start_marker = None
        linecount += 1
...
def start_line(event):
    ...
    global current_line, lastx, lasty, start_marker
    
    # Get canvas coordinates
    x = cv.canvasx(event.x)
    y = cv.canvasy(event.y)
    
    # Get snap point for start position if snapping is enabled and any snap tools are active
    if snap_var.get() and any(tool.get() for tool in active_snap_tools.values()):
        snapped_point = get_snap_point(event.x, event.y, is_final=True)
        if snapped_point:
            x, y = snapped_point
...
def update_line(event):
    ...
    if current_line:
        ...
        # Get snap point for preview if snapping is enabled and any snap tools are active
        if snap_var.get() and any(tool.get() for tool in active_snap_tools.values()):
            snapped_point = get_snap_point(event.x, event.y)
            if snapped_point:
                x, y = snapped_point
        
        # Update preview line
        cv.coords(current_line, lastx, lasty, x, y)
        cv.update_idletasks()  # Force immediate update for smoother preview

def end_line(event):
    """Finish drawing a line."""
    global current_line, start_marker, linecount
    ...
    if current_line:
        ...
        # Get snap point for end position if snapping is enabled and any snap tools are active
        if snap_var.get() and any(tool.get() for tool in active_snap_tools.values()):
            snapped_point = get_snap_point(event.x, event.y, is_final=True)
            if snapped_point:
                x, y = snapped_point
        ...
        cv.create_line(
            lastx, lasty, x, y,
            fill=hexstr, width=2,
            tags=(f"shape_{linecount}", "line")
        )
        
        # Clean up temporary elements
        cv.delete(current_line)
        cv.delete(start_marker)
        cv.delete('snap_indicator')  # Only clear the final snap indicator
...
def update_freehand(event):
    """Update freehand drawing preview."""
    global lastx, lasty, current_line_points
    if lastx is not None and lasty is not None:
        x = cv.canvasx(event.x)
        y = cv.canvasy(event.y)
        
        # Get snap point if snapping is enabled and any snap tools are active
        if snap_var.get() and any(tool.get() for tool in active_snap_tools.values()):
            snapped_point = get_snap_point(event.x, event.y)
            if snapped_point:
                x, y = snapped_point
...
# Add Snap Tools button
snap_tools_button = tk.Button(win, text="Snap Tools", bd=2, height=1, width=14, fg="white", bg="#263d42", command=lambda: open_snap_tools_window())
snap_tools_button.place(x=1020, y=24)  # Positioned below Markers button

def open_snap_tools_window():
    global snap_tools_window
    
    if snap_tools_window is not None:
        snap_tools_window.lift()
        return
        
    snap_tools_window = tk.Toplevel(win)
    snap_tools_window.title("Snap Tools")
    snap_tools_window.configure(bg="#263d42")
    snap_tools_window.geometry('200x300')
    
    # Main snap toggle
    snap_frame = tk.Frame(snap_tools_window, bg="#263d42")
    snap_frame.pack(fill=tk.X, padx=5, pady=5)
    snap_check = tk.Checkbutton(snap_frame, text="Enable Snapping", variable=snap_var,
                               bg="#263d42", fg="white", selectcolor="black",
                               font=('Arial', 10, 'bold'))
    snap_check.pack(side=tk.LEFT)
    
    # Snap tools
    tools_frame = tk.Frame(snap_tools_window, bg="#263d42")
    tools_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
    
    for tool_name, var in active_snap_tools.items():
        check = tk.Checkbutton(tools_frame, text=tool_name, variable=var,
                              bg="#263d42", fg="white", selectcolor="black",
                              font=('Arial', 9))
    ...
    def on_window_close():
        global snap_tools_window
        snap_tools_window.destroy()
        snap_tools_window = None
        
    snap_tools_window.protocol("WM_DELETE_WINDOW", on_window_close)
...
def select_tool(tool_name):
    """Select a drawing tool and update cursor."""
    global current_tool, previous_tool
    ...
    for name, button in tool_buttons.items():
        if name == tool_name:
            ...
            button.configure(bg="#263d42")
    
    # Unbind all drawing events first
    cv.unbind('<Button-1>')
    cv.unbind('<B1-Motion>')
    cv.unbind('<ButtonRelease-1>')
    cv.unbind('<Motion>')
    
    # Set cursor and bind events based on tool type
    if tool_name in ['line', 'circle', 'rectangle', 'polygon', 'arc']:
        # Set crosshair cursor for drawing tools
        cv.configure(cursor="crosshair")
        
        # First bind motion to show snap points BEFORE any other bindings
        cv.bind('<Motion>', show_drawing_snap_points)
        
        # Bind appropriate tool events
        if tool_name == 'line':
            cv.bind('<Button-1>', start_line)
            cv.bind('<B1-Motion>', update_line)
            cv.bind('<ButtonRelease-1>', end_line)
        ...
        # For non-drawing tools, clear snap points and set arrow cursor
        cv.delete('potential_snap')
        cv.delete('snap_indicator')
        cv.configure(cursor="arrow")

def show_drawing_snap_points(event):
    """Show snap points while a drawing tool is active."""
    # Only show snap points for drawing tools and when snapping is enabled
    if not current_tool in ['line', 'circle', 'rectangle', 'polygon', 'arc'] or not snap_var.get():
        cv.delete('potential_snap')
        return
    ...
    # Scale-adjusted snap distance (10 pixels at 100% zoom for more precision)
    hover_distance = 10.0 / scale_factor
    
    # Clear previous potential snap points
    cv.delete('potential_snap')
    
    potential_points = []
    
    # Get all items near the cursor for more efficient checking
    nearby_items = cv.find_overlapping(x-hover_distance, y-hover_distance, 
                                     x+hover_distance, y+hover_distance)
    
    # Filter out temporary items and grid lines
    valid_items = [item for item in nearby_items 
                  if 'temp' not in cv.gettags(item) 
                  and 'grid_lines' not in cv.gettags(item)
                  and 'potential_snap' not in cv.gettags(item)
    ...
    # Check grid points ONLY if grid is visible AND grid snap is enabled
    if grid_var.get() and active_snap_tools['Grid Point'].get():
        try:
            grid_size = float(grid_size_var.get())
            grid_x = round(x / grid_size) * grid_size
            grid_y = round(y / grid_size) * grid_size
            d = distance((x, y), (grid_x, grid_y))
            if d < hover_distance:
                potential_points.append(('Grid Point', (grid_x, grid_y)))
        except ValueError:
            pass
    ...
    if active_snap_tools['Endpoint'].get():
        for item in valid_items:
            ...
            if len(coords) >= 4:
                # Check start point
                d = distance((x, y), (coords[0], coords[1]))
                if d < hover_distance:
                    potential_points.append(('Endpoint', (coords[0], coords[1])))
                # Check end point
                d = distance((x, y), (coords[-2], coords[-1]))
                if d < hover_distance:
                    potential_points.append(('Endpoint', (coords[-2], coords[-1])))
    ...
    if active_snap_tools['Center'].get():
        for item in valid_items:
            ...
            if len(coords) >= 4:
                ...
                if d < hover_distance:
                    potential_points.append(('Center', center))
                        ...
                        try:
                            ...
                            if abs(denominator) > 1e-10:
                                ...
                                if 0 <= t <= 1 and 0 <= u <= 1:
                                    ix = x1 + t * (x2 - x1)
                                    iy = y1 + t * (y2 - y1)
                                    d = distance((x, y), (ix, iy))
                                    if d < hover_distance:
                                        potential_points.append(('Intersection', (ix, iy)))
                        except:
                            continue
    
    # Show all potential snap points with larger, more visible indicators
    for snap_type, point in potential_points:
        # Create a more visible dot with highlight
        cv.create_oval(point[0]-8, point[1]-8, point[0]+8, point[1]+8,
                      fill='#FFD700',  # Gold color
                      outline='black',
                      width=2,
                      tags=('potential_snap', 'above_all'))
        # Add larger text label
        cv.create_text(point[0], point[1]-20, text=snap_type,
                      fill='#000080', font=('Arial', 12, 'bold'),
                      tags=('potential_snap', 'above_all'))
        
    # Force immediate update for smoother feedback
    cv.update_idletasks()

# Remove the incorrect motion unbinding at the end of the file
# try:
#     cv.unbind('<Motion>')
# except:
#     pass
