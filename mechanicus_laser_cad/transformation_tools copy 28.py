...
class TransformationTools:
    ...
    def setup_rotate_tools(self, parent):
        ...
        # Manual angle input frame
        angle_frame = ttk.Frame(parent, style='Custom.TFrame')
        angle_frame.pack(pady=10, padx=5, fill='x')
        
        # Angle entry
        ttk.Label(angle_frame, text="Angle (°):", 
                style='Custom.TLabel').grid(row=0, column=0, padx=5)
        self.angle_entry = ttk.Entry(angle_frame, width=8)
        self.angle_entry.insert(0, "0.0")
        self.angle_entry.grid(row=0, column=1, padx=5)
        
        # Apply angle button
        apply_btn = tk.Button(angle_frame, text="Apply",
                             command=self.apply_manual_rotation,
                             fg='white', bg='#263d42',
                             activebackground='#2ecc71',
                             activeforeground='white',
                             width=6)
    ...
    def clear_selection(self):
        ...
        if self.selected_shape:
            try:
                ...
                if self.canvas.type(self.selected_shape) == 'line':
                    self.canvas.itemconfig(self.selected_shape, 
        ...
        # Clear all selection-related items
        for tag in ['bounding_box', 'scale_handle', 'rotation_marker', 'angle_label']:
        ...
        # Reset scale entries if they exist
        if hasattr(self, 'scale_entry_x') and hasattr(self, 'scale_entry_y'):
            self.scale_entry_x.delete(0, tk.END)
            self.scale_entry_x.insert(0, "0.0")
            self.scale_entry_y.delete(0, tk.END)
            self.scale_entry_y.insert(0, "0.0")
    ...
    def on_rotate_click(self, event):
        ...
        if self.rotate_state == 'select_shape':
            ...
            exclude_tags = ['grid_lines', 'below_all', 'above_all', 'snap_point', 'snap_line', 
                           'temp_marker', 'radius_marker', 'center_marker', 'temp', 
                           'guide_point', 'guide_line', 'crosshair', 'machine_pos',
                           'bounding_box', 'scale_handle', 'rotation_marker']
            ...
            if shapes:
                ...
                # Click in empty space - clear selection and deactivate rotation mode
                self.clear_selection()
                self.clear_rotation_markers()
                if self.rotate_mode:
                    self.toggle_rotate_selection()  # This will reset the button and deactivate rotation mode
    ...
    def rotate_shape_around_point(self, shape_id, cx, cy, angle):
        ...
        if shape_type == 'arc':
            # For arcs, we need to:
            # 1. Rotate the bounding box around the rotation center
            # 2. Adjust the start angle based on both the rotation and the box movement
            x1, y1, x2, y2 = coords
            ...
            # Calculate current arc center and dimensions
            arc_center_x = (x1 + x2) / 2
            arc_center_y = (y1 + y2) / 2
            width = x2 - x1
            height = y2 - y1
            
            # Calculate new arc center position after rotation around rotation point
            dx = arc_center_x - cx
            dy = arc_center_y - cy
            new_center_x = cx + (dx * cos_a - dy * sin_a)
            new_center_y = cy + (dx * sin_a + dy * cos_a)
            ...
            # Update the arc's bounding box
            self.canvas.coords(shape_id, new_x1, new_y1, new_x2, new_y2)
            
            # Update the start angle
            # The start angle needs to be adjusted by the rotation angle
            new_start = start + angle
            
            # Keep the start angle in the range [0, 360)
            new_start = new_start % 360
            
            # Update the arc's angles
            self.canvas.itemconfig(shape_id, start=new_start)
            
        elif shape_type == 'oval':
            # For ovals, preserve the shape by rotating just the bounding box
            x1, y1, x2, y2 = coords
            center_x = (x1 + x2) / 2
            center_y = (y1 + y2) / 2
            width = x2 - x1
            ...
            # Calculate new bounding box center after rotation around rotation point
            dx = center_x - cx
            dy = center_y - cy
            new_center_x = cx + (dx * cos_a - dy * sin_a)
            new_center_y = cy + (dx * sin_a + dy * cos_a)
            
            # Set new coordinates maintaining width and height
            new_x1 = new_center_x - width/2
            new_y1 = new_center_y - height/2
            new_x2 = new_center_x + width/2
            new_y2 = new_center_y + height/2
            ...
            # For lines, rectangles, and polygons, rotate all points
            new_coords = []
            for i in range(0, len(coords), 2):
                x = coords[i]
                y = coords[i + 1]
    ...
    def apply_manual_rotation(self):
        """Apply manual rotation from angle input"""
        if not self.selected_shape:
            messagebox.showerror("Error", "No shape selected")
            return
