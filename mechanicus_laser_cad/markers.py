...
class MarkersWindow:
    def __init__(self, root, canvas):
        ...
        # Get grid state from canvas
        self.grid_var = getattr(canvas, 'grid_var', tk.BooleanVar(value=False))
    ...
    def find_nearest_snap_point(self, x, y):
        ...
        # Only try grid snap if both snap is enabled and grid is visible
        if self.snap_var.get() and len(self.canvas.find_withtag('grid_lines')) > 0:
            # Convert screen coordinates to canvas coordinates for grid snap
            canvas_x = x / self.canvas_scale
            canvas_y = y / self.canvas_scale
            
            # Apply grid snap using unscaled coordinates
            snapped_x = round(canvas_x / self.grid_size) * self.grid_size
            snapped_y = round(canvas_y / self.grid_size) * self.grid_size
            
            # Convert back to screen coordinates
            snapped_x = snapped_x * self.canvas_scale
            snapped_y = snapped_y * self.canvas_scale
            
            # Only return grid snap if it's different from the original point
            if abs(snapped_x - x) > 1 or abs(snapped_y - y) > 1:
    ...
    def on_mouse_move(self, event):
        if self.center_point_mode or self.line_mode:
            ...
            # Only try snapping if enabled
            if self.snap_var.get():
                snap_point, snap_type = self.find_nearest_snap_point(x, y)
                if snap_point:
                    x, y = snap_point
            
            # Show preview circle for both center point mode and line mode
            if self.center_point_mode or self.line_mode:
                # Show preview circle with fixed size
                preview_radius = 7  # Fixed 7 pixels
            ...
            # If we're in line mode and have a start point, show the line preview
            if self.line_mode and self.line_start_point:
                ...
                if self.preview_line:
                    ...
                    preview_radius = 4  # Fixed 4 pixels
                    if not hasattr(self, 'preview_start_point'):
                        self.preview_start_point = self.canvas.create_oval(
                            self.line_start_point[0] - preview_radius,
                            self.line_start_point[1] - preview_radius,
                            self.line_start_point[0] + preview_radius,
                            self.line_start_point[1] + preview_radius,
                            fill='#4CAF50',
                            outline='#4CAF50',
            ...
            # Clean up preview elements if not in any mode
            self.remove_preview()
    ...
    def on_canvas_click(self, event):
        if self.line_mode:
            ...
            if not self.line_start_point:
                ...
                # Keep showing the preview circle for end point
                self.preview_circle = None  # Reset so it will be recreated
            else:
            ...
            if shape_id:
                ...
                if 'center_point' in tags:
                    ...
                    if len(marker_parts) == 2:
                        ...
                        radius = 8  # Fixed 8 pixels
                        inner_radius = 3  # Fixed 3 pixels
                    ...
                    if len(marker_parts) == 3:
                        ...
                        endpoint_radius = 4  # Fixed 4 pixels
    ...
    def add_center_marker(self, x, y):
        ...
        # Convert canvas coordinates to screen coordinates
        screen_x = x / self.canvas_scale
        screen_y = y / self.canvas_scale
        ...
        canvas_x = screen_x * self.canvas_scale
        canvas_y = screen_y * self.canvas_scale
        canvas_radius = radius * self.canvas_scale
        canvas_inner_radius = inner_radius * self.canvas_scale
    ...
    def add_line_marker(self, x1, y1, x2, y2):
        ...
        # Convert canvas coordinates to screen coordinates
        screen_x1 = x1 / self.canvas_scale
        screen_y1 = y1 / self.canvas_scale
        screen_x2 = x2 / self.canvas_scale
        screen_y2 = y2 / self.canvas_scale
        
        # Fixed size in screen pixels
        radius = 4
        
        # Convert back to canvas coordinates for placement
        canvas_x1 = screen_x1 * self.canvas_scale
        canvas_y1 = screen_y1 * self.canvas_scale
        canvas_x2 = screen_x2 * self.canvas_scale
        canvas_y2 = screen_y2 * self.canvas_scale
        canvas_radius = radius * self.canvas_scale
        
        # Create endpoint markers
        start_point = self.canvas.create_oval(
            canvas_x1 - canvas_radius, canvas_y1 - canvas_radius,
            canvas_x1 + canvas_radius, canvas_y1 + canvas_radius,
            fill='red',
            outline='red',
        ...
        end_point = self.canvas.create_oval(
            canvas_x2 - canvas_radius, canvas_y2 - canvas_radius,
            canvas_x2 + canvas_radius, canvas_y2 + canvas_radius,
            fill='red',
            outline='red',
            tags=('line_marker', 'marker', 'all_lines', shape_id)
