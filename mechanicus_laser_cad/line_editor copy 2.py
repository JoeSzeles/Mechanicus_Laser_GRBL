...
class LineEditor:
    ...
    def find_intersection(self):
        ...
        if len(self.selected_lines) == 2:
            self.canvas.delete('intersection')
            line1_coords = self.canvas.coords(self.selected_lines[0])
            line2_coords = self.canvas.coords(self.selected_lines[1])
            ...
            #messagebox.showinfo("Selection Complete", "Lines selected successfully. You can now apply fillet.")
    ...
    def on_trim_select(self, event):
        ...
        # Filter out grid lines, crosshairs, and non-line items more strictly
        exclude_tags = ['grid', 'crosshair', 'intersection', 'axis']
        lines = []
        for item in items:
    ...
    def on_trim_mid_select(self, event):
        ...
        # Filter out grid lines, crosshairs, and non-line items
        exclude_tags = ['grid', 'crosshair', 'intersection', 'axis']
        lines = []
        for item in items:
            if self.canvas.type(item) == 'line':
    ...
    def show_tools_window(self):
        ...
        if self.tools_window is None or not tk.Toplevel.winfo_exists(self.tools_window):
            ...
            self.tools_window.geometry("200x400")
            ...
            radius_frame = ttk.Frame(self.tools_window)
            radius_frame.pack(pady=5, padx=5, fill='x')
            
            ttk.Label(radius_frame, text="Size (mm):", 
                     foreground="white", background="#263d42").pack(side='left', padx=5)
            self.radius_entry = ttk.Entry(radius_frame, width=10)
