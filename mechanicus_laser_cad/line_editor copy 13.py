...
class LineEditor:
    ...
    def clear_selection(self):
        ...
        if hasattr(self, 'rotate_shape') and self.rotate_shape:
            # Reset appearance of rotated shape
            self.canvas.itemconfig(self.rotate_shape, fill='black', width=1)
            self.rotate_shape = None
            self.rotation_point = None
            self.original_coords = None
            self.start_angle = None
        ...
        for line_id in self.selected_lines[:]:
            try:
                if self.canvas.type(line_id) == 'arc':
    ...
    def on_rotate_click(self, event):
        ...
        if not shapes:
            ...
            if self.rotate_shape:
                # Reset previous shape appearance
                shape_type = self.canvas.type(self.rotate_shape)
                if shape_type == 'line':
                    self.canvas.itemconfig(self.rotate_shape, fill=self.original_colors.get(self.rotate_shape, 'black'), width=1)
                else:
                    self.canvas.itemconfig(self.rotate_shape, outline=self.original_colors.get(self.rotate_shape, 'black'), width=1)
            self.rotate_shape = None
            self.rotation_point = None
            self.original_coords = None
            self.start_angle = None
        ...
        if closest_shape:
            # If selecting a different shape, reset the previous one
            ...
            if self.rotate_shape not in self.original_colors:
                if shape_type == 'line':
                ...
                self.original_widths[self.rotate_shape] = self.canvas.itemcget(self.rotate_shape, 'width')
            ...
            # Find nearest midpoint using snaptools
            from snaptools import snap_to_midpoints
            midpoint = snap_to_midpoints(self.canvas, x, y)
            if midpoint:
                center = midpoint
            else:
                # Fallback to shape center if no midpoint found
                center = self.get_shape_center(self.rotate_shape)
            
            # Create center marker at midpoint
            ...
            if shape_type == 'rectangle':
                ...
                if not hasattr(self, 'temp_polygon'):
                    ...
                    self.temp_polygon = self.canvas.create_polygon(
                        *rotated_points,
                        fill=fill if fill != '' else None,  # Preserve original fill
                        outline='green',  # Keep outline green while selected
                        width=width,
                        tags=tags
                    )
                    # Store original properties for the new polygon
                    self.original_colors[self.temp_polygon] = outline
                    self.original_widths[self.temp_polygon] = width
