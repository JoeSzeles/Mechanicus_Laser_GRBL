...
class LineEditor:
    ...
    def deactivate_selection(self):
        ...
        self.set_active_tool(None)  # Reset active tool
        self.canvas.unbind('<Button-1>')
    ...
    def on_canvas_click(self, event):
        ...
        # Base tolerance divided by scale factor to make selection easier at high zoom
    ...
    def execute_fillet(self):
        ...
        try:
            ...
            center_marker_h = self.canvas.create_line(
                ...
                tags=('permanent_center_marker', 'no_engrave')  # Changed from 'center_marker' to 'permanent_center_marker'
            )
    ...
    def on_rotate_click(self, event):
        ...
        if self.rotate_state == 'select_shape':
            # Find shapes near click point
            items = self.canvas.find_overlapping(x-tolerance, y-tolerance, x+tolerance, y+tolerance)
            
            # Filter for valid shapes
            exclude_tags = ['grid_lines', 'below_all', 'above_all', 'snap_point', 'snap_line', 
                          'temp_marker', 'radius_marker', 'center_marker', 'temp', 
                          'guide_point', 'guide_line', 'crosshair', 'machine_pos',
                          'rotation_marker']
            ...
            if shapes:
                # Select the first valid shape
                self.rotate_shape = shapes[0]
                # Store original properties and coordinates
                if self.canvas.type(self.rotate_shape) == 'arc':
                    self.original_colors[self.rotate_shape] = self.canvas.itemcget(self.rotate_shape, 'outline')
                else:
                    self.original_colors[self.rotate_shape] = self.canvas.itemcget(self.rotate_shape, 'fill')
                self.original_width = self.canvas.itemcget(self.rotate_shape, 'width')
                self.original_coords = list(self.canvas.coords(self.rotate_shape))
                # Highlight the shape outline in green
                if self.canvas.type(self.rotate_shape) == 'arc':
                    self.canvas.itemconfig(self.rotate_shape, outline='green', width=2)
                else:
                    self.canvas.itemconfig(self.rotate_shape, outline='green', width=2)
                # Move to next state
                self.rotate_state = 'select_point'
                
        elif self.rotate_state == 'select_point':
    ...
    def get_current_angle(self, event):
        ...
        return math.degrees(math.atan2(
            event.y - self.rotation_point[1],
            event.x - self.rotation_point[0]
        ))

    def on_rotate_drag(self, event):
        """Handle dragging for rotation"""
        if self.rotate_state == 'rotate' and self.rotate_shape and self.rotation_point and self.start_angle is not None:
            # Calculate current angle
            current_angle = self.get_current_angle(event)
            
            # Calculate rotation angle with increased sensitivity (2x original)
            rotation_angle = (current_angle - self.start_angle) * 2
            
            # If angle snap is enabled, snap to nearest 5 degrees
            if self.angle_snap.get():
                rotation_angle = round(rotation_angle / 5.0) * 5.0
    ...
    def on_rotate_release(self, event):
        ...
        if self.rotate_state == 'rotate' and self.rotate_shape:
            ...
            # Reset shape appearance and clear markers
            if self.canvas.type(self.rotate_shape) == 'arc':
                self.canvas.itemconfig(
                    self.rotate_shape,
            ...
            # Clear rotation markers
            if self.rotation_marker:
                for marker in self.rotation_marker:
                    self.canvas.delete(marker)
                self.rotation_marker = None
    ...
    def apply_manual_rotation(self, direction):
        ...
        try:
            ...
            if direction == 'cw':
                angle = -angle  # Clockwise is negative
            ...
            self.canvas.itemconfig(self.rotate_shape, width=original_width)
    ...
    def rotate_shape_around_point(self, shape_id, cx, cy, angle):
        ...
        original_width = self.canvas.itemcget(shape_id, 'width')
        
        if shape_type in ['rectangle', 'oval', 'arc']:
            # For shapes defined by bounding box, convert to corner points
            x1, y1, x2, y2 = coords
            # For ovals and arcs, maintain aspect ratio by using the original coordinates
            if shape_type in ['oval', 'arc']:
            ...
            if shape_type == 'rectangle':
                # For rectangles, create a polygon with the rotated points
                if not hasattr(self, 'temp_polygon'):
                    # Convert rectangle to polygon on first rotation
                    self.temp_polygon = self.canvas.create_polygon(
                        *rotated_points,
                        fill=self.original_colors.get(shape_id, ''),
                        outline='red',
                        width=original_width,
                        tags=self.canvas.gettags(shape_id)
                    )
                    self.canvas.delete(shape_id)  # Delete original rectangle
                    self.rotate_shape = self.temp_polygon  # Update reference
                else:
                    # Update existing polygon
                    self.canvas.coords(self.temp_polygon, *rotated_points)
            else:
        ...
        # Restore original width
        if hasattr(self, 'temp_polygon'):
            self.canvas.itemconfig(self.temp_polygon, width=original_width)
        else:
            self.canvas.itemconfig(shape_id, width=original_width)

    def get_shape_center(self, shape_id):
        """Get the center point of a shape"""
        coords = list(self.canvas.coords(shape_id))
        if self.canvas.type(shape_id) == 'line':
            ...
            # For other shapes, average all points
            x_coords = coords[::2]
            y_coords = coords[1::2]
            return (
    ...
    def create_rotation_markers(self, x, y):
        """Create visual markers for rotation point"""
        marker_size = 6
        outer_size = 10
        self.rotation_marker = []
        
        # Create outer circle
        self.rotation_marker.append(
            self.canvas.create_oval(
                x - outer_size, y - outer_size,
                x + outer_size, y + outer_size,
                outline='blue', width=1,
                tags='rotation_marker'
            )
        )
        
        # Create cross
        self.rotation_marker.extend([
            self.canvas.create_line(  # Horizontal line
                x - marker_size, y,
                x + marker_size, y,
                fill='blue', width=2,
                tags='rotation_marker'
            ),
            self.canvas.create_line(  # Vertical line
                x, y - marker_size,
                x, y + marker_size,
                fill='blue', width=2,
                tags='rotation_marker'
    ...
    def set_rotation_point(self):
        ...
        self.create_rotation_markers(x, y)
        # Store starting angle for relative rotation
        self.start_angle = self.get_current_angle(x, y)
        # Move to rotation state
        self.rotate_state = 'rotate'

    def get_current_angle(self, x, y):
        """Calculate current angle relative to rotation point"""
        if not self.rotation_point:
            return 0
        return math.degrees(math.atan2(
