...
def get_snap_point(x, y, is_final=False):
    ...
    try:
        # Get base grid spacing and adjust for scale
        base_spacing = float(grid_size_var.get())
        if base_spacing <= 0:  # Add safety check
            base_spacing = 50  # Default value
    except:
        base_spacing = 50  # Default value
    ...
    x_grid = round(x / scaled_spacing) * scaled_spacing
    y_grid = round(y / scaled_spacing) * scaled_spacing
    
    # If this is a final point (like end of line), create a visible marker
    if is_final and snap_var.get():
        # Remove any existing snap markers
        cv.delete('snap_point')
        # Create new snap point marker
        cv.create_oval(x_grid-4, y_grid-4, x_grid+4, y_grid+4,
                      fill='red', outline='white', tags='snap_point')
...
def update_line(event):
    ...
    if lastx is not None and lasty is not None:
        ...
        # Always snap to grid if enabled
        if snap_var.get():
...
def end_line(event):
    global lastx, lasty, current_line, start_marker, linecount
    if lastx is not None and lasty is not None:
        x = cv.canvasx(event.x)
        y = cv.canvasy(event.y)
        ...
        # Create the final line with snapped endpoint
        cv.create_line(lastx, lasty, x, y,
                      fill=hexstr,
                      width=int(brush.get("1.0", "end-1c")),
                      tags=('all_lines', current_shape_id))
...
def update_snap_indicator(event):
    global snap_indicator
    x, y = cv.canvasx(event.x), cv.canvasy(event.y)

    # Check for snap to grid
    grid_x, grid_y = snap_to_grid(x, y, grid_size=20)
    snap_point = (grid_x, grid_y)

    # Check for snap to endpoints
    endpoint_snap = snap_to_endpoints(cv, x, y)
    if endpoint_snap:
        snap_point = endpoint_snap

    # Update or create the snap indicator
    if snap_indicator is None:
        snap_indicator = cv.create_oval(snap_point[0] - 3, snap_point[1] - 3, snap_point[0] + 3, snap_point[1] + 3, fill='red', outline='white', tags='snap_indicator')
    else:
        cv.coords(snap_indicator, snap_point[0] - 3, snap_point[1] - 3, snap_point[0] + 3, snap_point[1] + 3)

# Bind the update_snap_indicator function to mouse motion
...
def open_snap_tool_window():
    ...
    snap_window.geometry("200x100+1020+24")
    snap_window.configure(bg="#263d42")

    # Grid Snap Checkbox
    grid_snap_var = tk.BooleanVar(value=grid_var.get())
    grid_snap_checkbox = tk.Checkbutton(snap_window, text="Grid Snap", variable=grid_snap_var, bg="#263d42", fg="white", selectcolor="#263d42")
    grid_snap_checkbox.pack(anchor=tk.W)

    # Endpoint Snap Checkbox
    endpoint_snap_var = tk.BooleanVar(value=False)
    endpoint_snap_checkbox = tk.Checkbutton(snap_window, text="Endpoint Snap", variable=endpoint_snap_var, bg="#263d42", fg="white", selectcolor="#263d42")
    endpoint_snap_checkbox.pack(anchor=tk.W)

    def apply_snap_settings():
        grid_var.set(grid_snap_var.get())
        # Implement logic to enable/disable endpoint snapping based on endpoint_snap_var

    apply_button = tk.Button(snap_window, text="Apply", command=apply_snap_settings, bg="#263d42", fg="white")
    apply_button.pack(anchor=tk.W)
