...
root = tk.Tk()
root.title("MECHANICUS V.0.1 Beta. (c)Reservoir Frogs 2025")
...
snap_to_midpoints_var = tk.BooleanVar(value=True)  # Default to False for midpoint snapping
snap_to_centers_var=tk.BooleanVar(value=True)
...
# Global variable for group selection
selected_shapes = []
# Global variable for group bounding box
bounding_box = None
...
# Import undo functions from new module
from undoredo import init_canvas_history, save_canvas_state, undo, redo, restore_canvas_state, set_canvas, set_grid_var, set_scale_var, set_grid_size_var
...
def on_canvas_click(event):
    """Handle canvas clicks with group awareness"""
    ...
    shapes = []
    for item in items:
        if cv.type(item) in ['line', 'arc', 'oval', 'polygon', 'rectangle']:
            tags = cv.gettags(item)
            if not any(tag in tags for tag in exclude_tags):
                shapes.append(item)
    
    if shapes:
        # Get all members of the group if the clicked shape is part of a group
        group_members = get_group_members(shapes[0])
        
        # Clear previous selection
        clear_selection()
        
        # Highlight all members of the group
        highlight_group(group_members)
        
        # Store the selected group
        global selected_shapes
        selected_shapes = group_members
        
        # Create bounding box for the entire group
        create_group_bounding_box(group_members)
    else:
...
def select_line(event):
    ...
    # Clear previous selection
    if selected_line_id is not None:
        # Get the original color from the global hexstr variable
    ...
    # Filter for selectable items and get the topmost one
    selected_items = []
    for item in items:
        tags = cv.gettags(item)
        if 'all_lines' in tags and 'preview_line' not in tags:
            # Check if this item has a shape tag
            shape_tag = None
            for tag in tags:
                if tag.startswith('shape_'):
                    ...
                    break
            if shape_tag:
                selected_items.append((item, shape_tag))
    
    # If we found any items, select the topmost one
    if selected_items:
        selected_item, shape_tag = selected_items[-1]  # Last item is topmost
        selected_line_id = selected_item
        move_start_x = x
        move_start_y = y
        
        # Highlight based on item type
        if cv.type(selected_line_id) in ['oval', 'rectangle', 'polygon', 'arc']:
            cv.itemconfig(selected_line_id, outline='red')
        else:
            cv.itemconfig(selected_line_id, fill='red')
        
        # Raise all items with the same shape tag to maintain relative z-order
...
def toggle_tool(tool_name):
    global current_tool, current_shape_id, linecount, lastx, lasty, current_line_points
    ...
    # Set active tool button to green
    tool_buttons[tool_name].configure(bg="#00FF00")
    
    # Use select_tool for consistent tool state management
    select_tool(tool_name.lower())
...
def update_circle(event):
    ...
    if current_circle:
        ...
        # Check endpoint snapping only if enabled
        if snap_to_endpoints_var.get():
            endpoint = snap_to_endpoints(cv, x, y, tolerance=20.0)
            if endpoint:
        ...
        # Calculate radius using snapped coordinates
        radius = ((snap_x - lastx) ** 2 + (snap_y - lasty) ** 2) ** 0.5
        ...
        # Update radius marker at snapped position
        cv.delete('radius_marker')
        cv.create_oval(snap_x-4, snap_y-4, snap_x+4, snap_y+4,
...
def finish_circle(event):
    ...
    if current_circle:
        ...
        # Calculate final radius using snapped coordinates
        radius = ((snap_x - lastx) ** 2 + (snap_y - lasty) ** 2) ** 0.5
        cv.coords(current_circle,
                 lastx - radius, lasty - radius,
                 lastx + radius, lasty + radius)
...
def start_rectangle(event):
    global lastx, lasty, current_rectangle, current_shape_id, linecount
    x = cv.canvasx(event.x)
    y = cv.canvasy(event.y)
...
def update_rectangle(event):
    ...
    if current_rectangle:
        ...
        # Update rectangle coordinates using snapped position
        cv.coords(current_rectangle, lastx, lasty, snap_x, snap_y)
        cv.tag_raise('above_all')
...
def finish_rectangle(event):
    ...
    if current_rectangle:
        ...
        # Update rectangle with final snapped coordinates
        cv.coords(current_rectangle, lastx, lasty, snap_x, snap_y)
...
def finish_polygon(event):
    ...
    if current_polygon:
        ...
        # Check endpoint snapping only if enabled
        if snap_to_endpoints_var.get():
            endpoint = snap_to_endpoints(cv, x, y, tolerance=40.0)
            if endpoint:
                snap_x, snap_y = endpoint
        
        # Check grid snapping if enabled and no endpoint snap
        if snap_var.get() and grid_var.get():
            snap_x, snap_y = get_snap_point(x, y, is_final=True)
        
        # Get the coordinates of the polygon
        coords = cv.coords(current_polygon)
        
        # Add the final point
        coords.extend([snap_x, snap_y])
        
        # Create the final polygon
        cv.create_polygon(coords, fill='', outline='black',
                        tags=('shape', f'shape_{linecount}', 'all_lines'))
        
        # Clean up
        cv.delete(current_polygon)
...
def start_line(event):
    global current_line, lastx, lasty, start_x, start_y, current_shape_id, linecount
    ...
    x, y = event.x, event.y
    snap_x, snap_y = x, y  # Default to mouse position
    snap_found = False
    
    # Initialize current_shape_id for this line
    current_shape_id = f"shape_{linecount}"
    
    # Check endpoint snapping first if enabled
    if snap_to_endpoints_var.get():
        endpoint = snap_to_endpoints(cv, x, y, tolerance=40.0)
        if endpoint:
            snap_x, snap_y = endpoint
            snap_found = True
            # Create endpoint snap indicator
            cv.create_polygon(snap_x-6, snap_y+6, snap_x+6, snap_y+6, snap_x, snap_y-6,
                            fill='red', outline='white',
                            tags=('snap_indicator', 'above_all'))
            cv.create_text(snap_x, snap_y-15, text="Endpoint",
    ...
    # Check midpoint snapping if enabled and no endpoint found
    if not snap_found and snap_to_midpoints_var.get():
        midpoint = snap_to_midpoints(cv, x, y, tolerance=40.0)
        if midpoint:
            snap_x, snap_y = midpoint
            snap_found = True
            # Create midpoint snap indicator
            cv.create_polygon(snap_x-6, snap_y+6, snap_x+6, snap_y+6, snap_x, snap_y-6,
                            fill='#0088FF', outline='white',
                            tags=('snap_indicator', 'above_all'))
            cv.create_text(snap_x, snap_y-15, text="Midpoint",
                         fill='#0088FF', font=('Arial', 8, 'bold'),
                         tags=('snap_label', 'above_all'))
            
    # Check centerpoint snapping if enabled and no endpoint found
    if not snap_found and snap_to_centers_var.get():
        center = snap_to_centers(cv, x, y, tolerance=40.0)
        if center:
            snap_x, snap_y = center
            snap_found = True
            # Create center snap indicator
            cv.create_polygon(snap_x-6, snap_y+6, snap_x+6, snap_y+6, snap_x, snap_y-6,
                            fill='#0088FF', outline='white',
                            tags=('snap_indicator', 'above_all'))
            cv.create_text(snap_x, snap_y-15, text="Center",
                         fill='#0088FF', font=('Arial', 8, 'bold'),
                         tags=('snap_label', 'above_all'))
    
    # Check grid snapping if enabled and no other snap found
    if not snap_found and snap_var.get() and grid_var.get():
        snap_x, snap_y = get_snap_point(x, y, is_final=False)
        # Create grid snap indicator
        cv.create_polygon(snap_x-6, snap_y+6, snap_x+6, snap_y+6, snap_x, snap_y-6,
                         fill='green', outline='white',
                         tags=('snap_indicator', 'above_all'))
        cv.create_text(snap_x, snap_y-15, text="Grid",
                      fill='green', font=('Arial', 8, 'bold'),
                      tags=('snap_label', 'above_all'))
    
    # Store the snapped starting position
    lastx, lasty = snap_x, snap_y
    start_x, start_y = snap_x, snap_y
    
    # Create the start marker
    cv.create_polygon(start_x-6, start_y+6, start_x+6, start_y+6, start_x, start_y-6,
                     fill='green', outline='white',
                  tags=('start_marker', 'above_all'))
    
    # Create the preview line
    current_line = cv.create_line(start_x, start_y, start_x, start_y,
                                fill='gray', dash=(4, 4),
                                tags=('preview_line', 'above_all'))
    
    # Ensure all indicators are on top
    cv.tag_raise('above_all')
...
def update_line(event):
    ...
    if lastx is not None and lasty is not None:
        ...
        # Always check for endpoint and midpoint snapping first, then grid if enabled
        snap_x, snap_y = x, y
        snap_found = False
        
        # Clean up previous indicators
        cv.delete('snap_indicator')
        cv.delete('preview_line')  # Delete preview line before creating new indicators
        
        # Check endpoint snapping
        if snap_to_endpoints_var.get():
            endpoint = snap_to_endpoints(cv, x, y, tolerance=40.0)  # Increased tolerance
            if endpoint:
                snap_x, snap_y = endpoint
                # Show red indicator for snap point
                cv.create_oval(snap_x-4, snap_y-4, snap_x+4, snap_y+4,
        ...
        if not snap_found and snap_to_midpoints_var.get():
            midpoint = snap_to_midpoints(cv, x, y, tolerance=40.0)  # Increased tolerance
            if midpoint:
                snap_x, snap_y = midpoint
                # Create blue triangle marker for midpoint
                tri_size = 6  # Slightly larger triangle
                tri_points = [
                    snap_x, snap_y - tri_size,  # Top point
                    snap_x - tri_size, snap_y + tri_size,  # Bottom left
                    snap_x + tri_size, snap_y + tri_size   # Bottom right
                ]
                cv.create_polygon(tri_points,
                                fill='#0088FF', outline='white',
                                tags=('snap_indicator', 'above_all'))
                cv.create_text(snap_x, snap_y-15, text="Midpoint", 
                             fill='#0088FF', font=('Arial', 8, 'bold'),
        ...
        if not snap_found and snap_to_centers_var.get():
            center = snap_to_centers(cv, x, y, tolerance=40.0)  # Increased tolerance
            if center:
                snap_x, snap_y = center
                # Create blue triangle marker for center
                tri_size = 6  # Slightly larger triangle
                tri_points = [
                    snap_x, snap_y - tri_size,  # Top point
                    snap_x - tri_size, snap_y + tri_size,  # Bottom left
        ...
        # Check grid snapping if enabled and no other snaps occurred
        if not snap_found and snap_var.get() and grid_var.get():
            snap_x, snap_y = get_snap_point(x, y, is_final=False)
        ...
        cv.tag_raise('preview_line')
        cv.tag_raise('snap_indicator')
        cv.tag_raise('start_marker')
...
def end_line(event):
    ...
    if lastx is not None and lasty is not None:
        ...
        # Check endpoint snapping
        if snap_to_endpoints_var.get():
            endpoint = snap_to_endpoints(cv, x, y, tolerance=30.0)  # Increased tolerance
            if endpoint:
                snap_x, snap_y = endpoint
                snap_found = True
        ...
        # Check midpoint snapping
        if not snap_found and snap_to_midpoints_var.get():
            midpoint = snap_to_midpoints(cv, x, y, tolerance=30.0)  # Increased tolerance
            if midpoint:
                snap_x, snap_y = midpoint
                snap_found = True
        ...
        # Check center snapping
        if not snap_found and snap_to_centers_var.get():
            center = snap_to_centers(cv, x, y, tolerance=30.0)  # Increased tolerance
            if center:
                snap_x, snap_y = center
                snap_found = True
...
def update_freehand(event):
    ...
    if lastx is not None and lasty is not None:
        ...
        # Add snapped point to the list
        current_line_points.append((snap_x, snap_y))
...
def open_drawing_tools():
    ...
    tools_window.geometry("240x220+1700+0")  # Adjusted size for grid layout
    ...
    for i, btn_text in enumerate(buttons):
        ...
        btn = Button(tools_window, text=btn_text, bd=2, height=1, width=12,
                    fg="white", bg="#263d42", 
    ...
    laser_active_checkbox.grid(row=4, column=0, columnspan=2, pady=5)
    ...
    z_axis_checkbox.grid(row=5, column=0, columnspan=2, pady=5)
...
#transform tools
transform_tools = TransformationTools(root, cv)
transform_tools.show_tools_window()
#Grouptools
group_tools = GroupTools(root, cv)
group_tools.show_tools_window()
...
def update_snap_indicator(event):
    ...
    # Only show snap indicators if we're using a tool that supports snapping
    if not current_tool or current_tool not in ['line', 'rectangle', 'circle', 'polygon', 'arc']:
        return
    ...
    # Check center snapping if enabled and no endpoint found
    if not snap_found and snap_to_centers_var.get():
        center = snap_to_centers(cv, x, y, tolerance=40.0)
        if center:
            snap_x, snap_y = center
...
def start_arc(event):
    global lastx, lasty, current_arc, current_shape_id, linecount
    x = cv.canvasx(event.x)
    y = cv.canvasy(event.y)
    ...
    # Initialize arc with 0 size
    current_arc = cv.create_arc(
        x, y, x, y,
        start=0, extent=90,  # Default arc angles
        outline=hexstr,
        width=int(brush.get("1.0", "end-1c")),
        tags=('all_lines', current_shape_id)
    )
...
def update_arc(event):
    ...
    if current_arc:
        ...
        # Calculate radius and angles using snapped coordinates
        radius = ((snap_x - lastx) ** 2 + (snap_y - lasty) ** 2) ** 0.5
        angle = math.degrees(math.atan2(snap_y - lasty, snap_x - lastx))
        
        # Update arc
        cv.coords(current_arc,
            lastx - radius, lasty - radius,
                 lastx + radius, lasty + radius)
        cv.itemconfig(current_arc, start=angle, extent=90)
        cv.tag_raise('above_all')

def finish_arc(event):
    global current_arc, linecount
    if current_arc:
        x = cv.canvasx(event.x)
        y = cv.canvasy(event.y)
        ...
        # Calculate final radius and angles using snapped coordinates
        radius = ((snap_x - lastx) ** 2 + (snap_y - lasty) ** 2) ** 0.5
        angle = math.degrees(math.atan2(snap_y - lasty, snap_x - lastx))
        
        # Update arc with final coordinates
        cv.coords(current_arc,
            lastx - radius, lasty - radius,
                 lastx + radius, lasty + radius)
        cv.itemconfig(current_arc, start=angle, extent=90)
...
def open_snap_tool_window():
    global snap_tool_window, snap_var, snap_to_endpoints_var, snap_to_midpoints_var, snap_to_centers_var
    
    # If window already exists, just lift it
    if 'snap_tool_window' in globals() and snap_tool_window is not None:
        try:
            snap_tool_window.lift()
            snap_tool_window.focus_force()
            return
        except:
            pass  # Window was probably destroyed
    
    snap_tool_window = tk.Toplevel(win)
    snap_tool_window.title("Snap Tools")
    snap_tool_window.geometry("200x150+1700+100")
    snap_tool_window.configure(bg="#263d42")
    
    # Create variables if they don't exist
    if 'snap_var' not in globals():
        snap_var = tk.BooleanVar(value=False)
    if 'snap_to_endpoints_var' not in globals():
        snap_to_endpoints_var = tk.BooleanVar(value=False)
    if 'snap_to_midpoints_var' not in globals():
        snap_to_midpoints_var = tk.BooleanVar(value=False)
    if 'snap_to_centers_var' not in globals():
        snap_to_centers_var = tk.BooleanVar(value=False)
    
    # Create checkboxes with proper styling
    grid_snap = tk.Checkbutton(snap_tool_window, text="Grid Snap", 
                              variable=snap_var,
                              bg="#263d42", fg="white", 
                              selectcolor="#263d42",
                              activebackground="#263d42",
                              activeforeground="white")
    grid_snap.pack(pady=5, anchor=W, padx=10)
    
    endpoint_snap = tk.Checkbutton(snap_tool_window, text="Endpoint Snap", 
                                  variable=snap_to_endpoints_var,
                                  bg="#263d42", fg="white", 
                                  selectcolor="#263d42",
                                  activebackground="#263d42",
                                  activeforeground="white")
    endpoint_snap.pack(pady=5, anchor=W, padx=10)
    
    midpoint_snap = tk.Checkbutton(snap_tool_window, text="Midpoint Snap", 
                                  variable=snap_to_midpoints_var,
                                  bg="#263d42", fg="white", 
                                  selectcolor="#263d42",
                                  activebackground="#263d42",
                                  activeforeground="white")
    midpoint_snap.pack(pady=5, anchor=W, padx=10)

    center_snap = tk.Checkbutton(snap_tool_window, text="Center Snap", 
                                variable=snap_to_centers_var,
                                bg="#263d42", fg="white", 
                                selectcolor="#263d42",
                                activebackground="#263d42",
                                activeforeground="white")
    center_snap.pack(pady=5, anchor=W, padx=10)
    
    # Make window stay on top
    snap_tool_window.attributes('-topmost', True)
...
def clear_selection():
    """Clear current selection with group awareness"""
    global selected_shapes, bounding_box
    
    # Unhighlight all selected shapes
    if selected_shapes:
        highlight_group(selected_shapes, False)
    
    # Clear bounding box
    if bounding_box:
        cv.delete(bounding_box)
        bounding_box = None
...
def select_tool(tool_name):
    global current_tool, current_shape
    
    # Reset any existing state
    cv.delete('preview_line', 'snap_indicator', 'snap_label', 'start_marker')
    current_shape = None
    
    # Update current tool
    current_tool = tool_name
    
    # Unbind all tool-specific events
    cv.unbind('<Button-1>')
    cv.unbind('<B1-Motion>')
    cv.unbind('<ButtonRelease-1>')
    
    # Bind the motion event for snap indicators
    cv.bind('<Motion>', update_snap_indicator)
    ...
    elif tool_name == 'paint' or tool_name == 'live carving':
        cv.bind('<Button-1>', activate_Paint_Gcode)
        cv.bind('<B1-Motion>', lambda event: Paint_Gcode(event, gcode))
        cv.bind('<ButtonRelease-1>', key_released)

def get_group_members(shape_id):
    """Get all members of a group that contains the given shape"""
    # Check all tags of the shape for group membership
    for tag in cv.gettags(shape_id):
        if tag.startswith('group_'):
            # Return all shapes with this group tag
            return [item for item in cv.find_withtag(tag)]
    return [shape_id]  # Return the shape itself if not in a group

def get_group_bounds(shape_ids):
    """Calculate the bounding box for a group of shapes"""
    if not shape_ids:
        return None
    
    # Initialize bounds with the first shape
    bounds = list(cv.bbox(shape_ids[0]))
    
    # Expand bounds to include all shapes
    for shape_id in shape_ids[1:]:
        shape_bbox = cv.bbox(shape_id)
        if shape_bbox:
            bounds[0] = min(bounds[0], shape_bbox[0])  # x1
            bounds[1] = min(bounds[1], shape_bbox[1])  # y1
            bounds[2] = max(bounds[2], shape_bbox[2])  # x2
            bounds[3] = max(bounds[3], shape_bbox[3])  # y2
    
    return bounds

def highlight_group(shape_ids, highlight=True):
    """Highlight or unhighlight all shapes in a group"""
    if not shape_ids:
        return
        
    for shape_id in shape_ids:
        try:
            shape_type = cv.type(shape_id)
            if highlight:
                if shape_type == 'line':
                    cv.itemconfig(shape_id, fill='green', width=2)
                else:
                    cv.itemconfig(shape_id, outline='green', width=2)
            else:
                # Restore original appearance from stored properties
                if shape_type == 'line':
                    cv.itemconfig(shape_id, 
                        fill=cv.itemcget(shape_id, 'fill'),
                        width=cv.itemcget(shape_id, 'width'))
                else:
                    cv.itemconfig(shape_id,
                        outline=cv.itemcget(shape_id, 'outline'),
                        fill=cv.itemcget(shape_id, 'fill'),
                        width=cv.itemcget(shape_id, 'width'))
        except tk.TclError:
            continue

def create_group_bounding_box(shape_ids):
    """Create a bounding box around a group of shapes"""
    bounds = get_group_bounds(shape_ids)
    if not bounds:
        return
    ...
    if bounding_box:
        cv.delete(bounding_box)
    bounding_box = cv.create_rectangle(
