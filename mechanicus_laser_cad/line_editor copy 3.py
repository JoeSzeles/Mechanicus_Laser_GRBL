...
class LineEditor:
    ...
    def on_canvas_click(self, event):
        ...
        lines = [item for item in items if self.canvas.type(item) == 'line' 
                and not any(tag in self.canvas.gettags(item) 
                for tag in ['grid', 'crosshair', 'intersection', 'axis', 'endpoint_marker', 'snap_marker'])]
    ...
    def on_adjust_click(self, event):
        ...
        lines = [item for item in items if self.canvas.type(item) == 'line' 
                and not any(tag in self.canvas.gettags(item) 
                for tag in ['grid', 'crosshair', 'intersection', 'axis'])]
