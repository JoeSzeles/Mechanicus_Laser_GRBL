...
def generate_shape_commands(shape_type, coords, canvas_width, canvas_height, bed_max_x, bed_max_y, 
                          draw_speed, laser_power, z_Draw, z_Travel, laser_active, z_active):
...
def Engrave(cv=None, ser=None, draw_speed_input=None, laser_power_input=None, layers_input=None, 
    ...
    for layer in range(layers):
        ...
        for shape_id, objects in shapes.items():
            ...
            coords = canvas.coords(first_obj)
            ...
            if shape_type == 'line' or shape_type == 'polygon':
                ...
                # Move to start position
                x, y = points[0]
                send_command(serial, f"G0 X{x:.3f} Y{y:.3f}", movement_wait)
                
                # Lower Z if active
                if z_active:
                    send_command(serial, f"G1 Z{z_Draw}", z_wait)
                
                # Turn on laser
                if laser_active:
                    send_command(serial, f"M3 S{laser_power}", laser_wait)
                
                # Draw all points with proper waits
                for x, y in points[1:]:
                    send_command(serial, f"G1 X{x:.3f} Y{y:.3f}", movement_wait)
                ...
                start_x = machine_center_x + machine_radius_x
                start_y = machine_center_y
                send_command(serial, f"G0 X{start_x:.3f} Y{start_y:.3f}", movement_wait)
                ...
                num_segments = max(72, int(abs(extent) / 5))  # Increased minimum segments for smoother arcs
                for i in range(num_segments + 1):
                    angle_rad = math.radians(start_angle + (extent * i / num_segments))
                    x = machine_center_x + machine_radius_x * math.cos(angle_rad)
                    y = machine_center_y + machine_radius_y * math.sin(angle_rad)
