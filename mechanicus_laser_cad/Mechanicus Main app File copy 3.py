...
def toggle_tool(tool_name):
    ...
    # If we have a markers window active, store its bindings state
    markers_active = False
    if hasattr(win, 'markers_window'):
        markers_active = win.markers_window.center_point_mode
        # Cancel center point mode if active
        if markers_active:
            win.markers_window.cancel_center_point_mode()
    ...
    if tool_name == active_tool:
        active_tool = None
        # Restore markers bindings if they were active
        if markers_active and hasattr(win, 'markers_window'):
            win.markers_window.restore_bindings()
        return
...
Engravebutton = Button(win, text="Engrave", bd=2, height=1, width=14, fg="white", bg="#263d42", 
                      command=lambda: engrave.Engrave(cv=cv, ser=ser, 
                                                    draw_speed_input=draw_speed_input,
                                                    laser_power_input=laser_power_input,
                                                    layers_input=layers_input,
                                                    laser_active_var=laser_active_var,
                                                    z_axis_active_var=z_axis_active_var))
